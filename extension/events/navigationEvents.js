...
function createNavigationSecurityEvent(hop, hopsList = []) {
  let tabId = 0;
  let targetUrl = "";
  ...
  const cleanHopsList = Array.isArray(hopsList) ? hopsList.filter((h) => typeof h === "string" && h.trim().length > 0) : [];
  ...
  try {
    ...
    const urlToParse = isStandardScheme ? targetUrl.startsWith("//") ? "https:" + targetUrl : targetUrl : targetUrl.includes("://") ? targetUrl : "https://" + targetUrl;
