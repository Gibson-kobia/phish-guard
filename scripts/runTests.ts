...
import { runThreeBugsRegressionTests } from '../tests/threeBugsRegression.test';
...
async function main() {
  ...
  console.log('--- 4. Running 3 Production Bugs Regression Suite ---');
  const bugsResults = await runThreeBugsRegressionTests();
  if (bugsResults.failed === 0) {
    console.log(`  ✅ All ${bugsResults.passed} Production Bug Regression tests passed!\n`);
  } else {
    for (const err of bugsResults.errors) {
      console.error(`  ❌ ${err}`);
    }
    console.error(`Production Bug Regression Tests: ${bugsResults.passed} passed, ${bugsResults.failed} failed\n`);
  }

  const totalTests = secResults.total + cloudResults.total + authResults.passed + authResults.failed + bugsResults.passed + bugsResults.failed;
  const totalPassed = secResults.passed + cloudResults.passed + authResults.passed + bugsResults.passed;
  const totalFailed = secResults.failed + cloudResults.failed + authResults.failed + bugsResults.failed;
