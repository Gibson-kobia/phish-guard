...
export const CustomerPortal: React.FC<CustomerPortalProps> = ({ onInspectUrl }) => {
  ...
  const [tokenSecrets, setTokenSecrets] = useState<Record<string, string>>({
    'tok_acme_pilot_01': 'ENROLL-ACME-PILOT-TOKEN'
  });
  ...
  const handleCopy = async (text: string, id: string): Promise<boolean> => {
    if (!text) {
      return false;
    }
    ...
    // Primary: modern navigator.clipboard API
    if (typeof navigator !== 'undefined' && navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
      try {
        await navigator.clipboard.writeText(text);
        success = true;
      } catch {
        // Fallback below if writeText rejected (e.g. iframe permission constraints)
        success = false;
      }
    }

    // Secondary / Fallback: document.execCommand('copy') via temporary textarea
    ...
    if (success) {
      setCopiedText(id);
      setTimeout(() => {
        setCopiedText(prev => (prev === id ? null : prev));
      }, 2000);
      return true;
    }

    // Do NOT fake copied state if operation failed
    return false;
  };
  ...
  const handleCreateToken = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.token?.id && data.token?.token) {
          setTokenSecrets(prev => ({
            ...prev,
            [data.token.id]: data.token.token
          }));
        }
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 mt-6 pt-5 border-t border-slate-800/80 overflow-x-auto text-xs">
          ...
          >
            ...
            <span>Deployment & Tokens ({activeTokensCount})</span>
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/40">
              ...
              <span className="text-xs text-slate-400">{activeTokensCount} active / {tokens.length} total tokens</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {tokens.map((t) => {
                    ...
                    const copyToken = t.token || tokenSecrets[t.id] || (t.id === 'tok_acme_pilot_01' ? 'ENROLL-ACME-PILOT-TOKEN' : '');
                    const displayToken = copyToken
                      ? (copyToken.length > 28 ? `${copyToken.slice(0, 14)}...${copyToken.slice(-6)}` : copyToken)
                      : (t.tokenHash ? `${t.tokenHash.slice(0, 14)}...` : '••••••••••••••••');
                    const usesDisplay = ((t as any).useCount ?? (t as any).usedCount ?? 0);
                    return (
                      <tr key={t.id || t.token} className="hover:bg-slate-800/40 transition-colors">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-blue-400 font-medium" title={copyToken ? 'Click copy icon to copy' : 'Plaintext secret not in session memory'}>{displayToken}</span>
                            <button
                              id={`copy-token-btn-${t.id}`}
                              onClick={() => handleCopy(copyToken, t.id)}
                              className="text-slate-400 hover:text-white p-1 rounded hover:bg-slate-800 transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                              title={copyToken ? "Copy Token" : "Token secret not available"}
                              disabled={!copyToken}
                            >
                              {copiedText === t.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                            </button>
                          </div>
                        </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    const activeTok = tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]));
                    const tokenToUse = activeTok ? (activeTok.token || tokenSecrets[activeTok.id]) : (tokenSecrets[tokens[0]?.id] || tokens[0]?.token || 'ENROLL-ACME-PILOT-TOKEN');
                    handleCopy(JSON.stringify({
                      "ExtensionSettings": {
                        "*": {
...
{JSON.stringify({
  ...
  "3rdparty": {
    "extensions": {
      "phishguard_enterprise": {
        "enrollmentToken": (tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))
          ? (tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))!.token || tokenSecrets[tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))!.id])
          : (tokenSecrets[tokens[0]?.id] || tokens[0]?.token || 'ENROLL-ACME-PILOT-TOKEN')),
        "organizationId": activeOrgId,
        "backendUrl": window.location.origin
      }
    }
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    const activeTok = tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]));
                    const tokenToUse = activeTok ? (activeTok.token || tokenSecrets[activeTok.id]) : (tokenSecrets[tokens[0]?.id] || tokens[0]?.token || 'ENROLL-ACME-PILOT-TOKEN');
                    handleCopy(`# PhishGuard Intune Enrollment Script
$RegPath = "HKLM:\\SOFTWARE\\Policies\\Google\\Chrome\\3rdparty\\extensions\\phishguard"
New-Item -Path $RegPath -Force | Out-Null
Set-ItemProperty -Path $RegPath -Name "EnrollmentToken" -Value "${tokenToUse}"
...
$RegPath = "HKLM:\\SOFTWARE\\Policies\\Google\\Chrome\\3rdparty\\extensions\\phishguard"
New-Item -Path $RegPath -Force | Out-Null
Set-ItemProperty -Path $RegPath -Name "EnrollmentToken" -Value "${(tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))
  ? (tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))!.token || tokenSecrets[tokens.find(t => t.status === 'ACTIVE' && (t.token || tokenSecrets[t.id]))!.id])
  : (tokenSecrets[tokens[0]?.id] || tokens[0]?.token || 'ENROLL-ACME-PILOT-TOKEN'))}"
Set-ItemProperty -Path $RegPath -Name "OrganizationId" -Value "${activeOrgId}"
