...
/**
 * Builds a chronological security activity timeline for an evaluated page
 */
export function buildSecurityTimeline(params: {
  url: string;
  domain: string;
  timestamp: number;
  score: number;
  ...
  // 3. Domain & Reputation Evaluation
  ...
  // 4. Form & DOM Security Inspection
  if (params.formMeta) {
  ...
  // 5. Social Engineering Inspection
  ...
  events.push({
    ...
    description: params.score < 20 
      ? 'Clean baseline security posture affirmed.'
      : `${params.signals.length} security signal(s) evaluated.`,
