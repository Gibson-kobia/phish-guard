/**
 * PhishGuard Download Security Event Analyzer
 */

import { DownloadSecurityEvent, ThreatSeverity } from './eventTypes';

export const EXECUTABLE_EXTENSIONS = [
  'exe', 'msi', 'bat', 'cmd', 'scr', 'vbs', 'js', 'jar', 'apk',
  'iso', 'img', 'vhd', 'hta', 'ps1', 'wsf', 'com', 'pif'
];
...
export function createDownloadSecurityEvent(
  ctxOrTabId: RawDownloadContext | number | null | undefined,
  originUrlArg?: string,
  downloadUrlArg?: string,
  filenameArg?: string,
  originRiskScoreArg?: number,
  downloadIdArg?: number
): DownloadSecurityEvent | null {
  let tabId = 0;
  let downloadId = 0;
  let url = '';
  let filename = '';
