...
export class JsonFileDatabaseAdapter implements IDatabaseAdapter {
  ...
  public getEnrollmentTokens(organizationId: string): EnrollmentToken[] {
    ...
    return this.data.enrollmentTokens
      ...
      .map(t => {
        let status: 'ACTIVE' | 'REVOKED' | 'EXPIRED' = t.status;
        if (t.status === 'REVOKED' || t.revokedAt !== null) {
          status = 'REVOKED';
        } else if (t.expiresAt && now > t.expiresAt) {
          status = 'EXPIRED';
        } else if (t.maxUses !== null && t.maxUses !== undefined && t.useCount >= t.maxUses) {
          status = 'EXPIRED';
        } else {
          status = 'ACTIVE';
        }
        return { ...t, status, token: '' };
      });
  }
